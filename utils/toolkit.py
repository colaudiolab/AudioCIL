...
def accuracy(y_pred, y_true, nb_old, init_cls, increment):
    ...
    #! our modify to adapt diferent incremental session 
    groups = np.arange(init_cls, np.max(y_true), increment)
    groups = np.concatenate(([0], groups))
    for class_id in groups:
        if class_id == 0:
            idxes = np.where(
                np.logical_and(y_true >= class_id, y_true < class_id + init_cls)
            )[0]
            label = "{}-{}".format(
                str(class_id).rjust(2, "0"), str(class_id + init_cls - 1).rjust(2, "0")
            )
        else:
            idxes = np.where(
